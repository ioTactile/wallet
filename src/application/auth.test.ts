...
describe('local PIN', () => {
  ...
  it('rejects a confirmation mismatch before writing', async () => {
    const vault = new InMemoryPinVault();
    await expect(new CreatePin(vault, new FakePinHasher()).execute('1234', '4321')).rejects.toBeInstanceOf(
      PinMismatch,
    );
...
describe('API session', () => {
  it('registers then logs out', async () => {
    ...
    const session = await new RegisterAccount(api, sessions).execute('jordan@example.com', 'longenough');
